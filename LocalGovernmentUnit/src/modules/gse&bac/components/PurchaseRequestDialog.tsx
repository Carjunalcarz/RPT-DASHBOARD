...
const PurchaseRequestDialog = ({
  ...
  // Reload sections when the selected RC-GLA changes by resolving its source rc_id
  useEffect(() => {
    const sourceRcId =
      budgetGlOptions.find((opt) => opt.rc_gla_id === rcId)?.rc_id || rcId;

    if (sourceRcId) {
      fetchSections(sourceRcId).then(setSections);
    } else {
      setSections([]);
      setRcsId("");
    }
  ...
  const loadDropdowns = async () => {
    setLoadingData(true);
    const options = await fetchPRBudgetGlAccountOptions();
    setBudgetGlOptions(options);
  ...
  return (
    ...
    >
      <div className="space-y-1">
        ...
        <SectionHeading>Budget GL Account</SectionHeading>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormSelect
            id="rc"
            label="GL Account (Budgetary)"
            value={rcId}
            onChange={setRcId}
            required
          ...
          >
            ...
            {budgetGlOptions.map((opt) => (
              <option key={opt.rc_gla_id} value={opt.rc_gla_id}>
                {opt.label} (Remaining:{" "}
                {opt.total_remaining.toLocaleString("en-PH", {
        ...
        {!isFormValid && (
          <p className="text-xs text-error mt-3">
            * Please fill in all required fields (date, GL account, purpose)
          </p>
        )}
      </div>
    </BaseDialog>
