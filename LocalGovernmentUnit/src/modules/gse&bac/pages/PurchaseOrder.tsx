...
import {
  ...
  upsertBudgetPendingDetailFromPO,
...
const generatePOPDF = (
  ...
  text(supplier?.description || "-", marginLeft + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });

  text("P.O. No.:", marginLeft + colWidth1 + 2, rowY + 4, { size: labelSize });
  text(po.po_no || "-", marginLeft + colWidth1 + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });

  text("Date:", marginLeft + colWidth1 + colWidth2 + 2, rowY + 4, {
    size: labelSize,
  });
  text(po.po_date || "-", marginLeft + colWidth1 + colWidth2 + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });
  ...
  text("Mode of Procurement:", marginLeft + colWidth1 + 2, rowY + 4, {
    size: labelSize,
  });
  text(modeProc?.description || "-", marginLeft + colWidth1 + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });
  ...
  text(supplier?.tin || "-", marginLeft + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });

  text("Contact:", marginLeft + 45, rowY + 4, { size: labelSize });
  text(supplier?.contact || "-", marginLeft + 45, rowY + 9, {
    bold: true,
    size: valueSize,
  });

  text("Abstract No.:", marginLeft + colWidth1 + 2, rowY + 4, {
    size: labelSize,
  });
  text(abstract?.a_no || "-", marginLeft + colWidth1 + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });

  text("PR No.:", marginLeft + colWidth1 + colWidth2 + 2, rowY + 4, {
    size: labelSize,
  });
  text(prNo || "-", marginLeft + colWidth1 + colWidth2 + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });
  ...
  text(po.place_of_delivery || "-", marginLeft + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });

  text("Date of Delivery:", marginLeft + colWidth1 + 2, rowY + 4, {
    size: labelSize,
  });
  text(po.date_of_delivery || "-", marginLeft + colWidth1 + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });

  text("Days to Deliver:", marginLeft + colWidth1 + colWidth2 + 2, rowY + 4, {
    size: labelSize,
  });
  text(
    po.days_to_deliver != null ? String(po.days_to_deliver) : "-",
    marginLeft + colWidth1 + colWidth2 + 2,
    rowY + 9,
    { bold: true, size: valueSize },
  );
  ...
  text(deliveryTerm?.description || "-", marginLeft + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });

  text("Payment Term:", marginLeft + colWidth1 + 2, rowY + 4, {
    size: labelSize,
  });
  text(paymentTerm?.description || "-", marginLeft + colWidth1 + 2, rowY + 9, {
    bold: true,
    size: valueSize,
  });
  ...
  text("QTY", tableX + col.qty / 2, y + 5.5, {
    align: "center",
    bold: true,
    size: 8,
  });
  ...
  text("UNIT", tableX + col.unit / 2, y + 5.5, {
    align: "center",
    bold: true,
    size: 8,
  });
  ...
  text("DESCRIPTION", tableX + col.desc / 2, y + 5.5, {
    align: "center",
    bold: true,
    size: 8,
  });
  ...
  text("UNIT COST", tableX + col.unitCost / 2, y + 5.5, {
    align: "center",
    bold: true,
    size: 8,
  });
  ...
  text("AMOUNT", tableX + col.amount / 2, y + 5.5, {
    align: "center",
    bold: true,
    size: 8,
  });
  ...
  lines.forEach((line, idx) => {
    ...
    doc.setFontSize(8);
    const descTextLines = doc.splitTextToSize(
    ...
    text(String(idx + 1), tableX + col.no / 2, y + 5, {
      align: "center",
      size: 8,
    });
    tableX += col.no;
    ...
    text(String(line.qty), tableX + col.qty / 2, y + 5, {
      align: "center",
      size: 8,
    });
    tableX += col.qty;
    ...
    text(line.unit_code.toUpperCase(), tableX + col.unit / 2, y + 5, {
      align: "center",
      size: 8,
    });
    tableX += col.unit;
    ...
    text(formatCurrency(line.unit_price), tableX + col.unitCost - 2, y + 5, {
      align: "right",
      size: 8,
    });
    tableX += col.unitCost;
    ...
    text(formatCurrency(line.total_amount), tableX + col.amount - 2, y + 5, {
      align: "right",
      bold: true,
      size: 8,
    });
  ...
  drawRect(
    tableX,
    y,
    col.no + col.qty + col.unit + col.desc + col.unitCost,
    subtotalRowHeight,
    headerBg,
  );
  text(
    "TOTAL AMOUNT",
    marginLeft + col.no + col.qty + col.unit + col.desc + col.unitCost - 2,
    y + 6.5,
  ...
  text(formatCurrency(subtotal), tableX + col.amount - 2, y + 6.5, {
    align: "right",
    bold: true,
    size: 10,
  });
  ...
  y += 4;
  text(po.issued_by || "", marginLeft + sigWidth / 2, y, {
    align: "center",
    bold: true,
    size: 9,
  });
  ...
  y += 4;
  text(po.received_by || "", rightColX + sigWidth / 2, y, {
    align: "center",
    bold: true,
    size: 9,
  });
...
const PurchaseOrder = () => {
  ...
  const openForm = async (po?: PO) => {
    ...
    const [abs, sups, dts, pts, mps, prs] = await Promise.all([
      awardedAbstracts.length
        ? Promise.resolve(awardedAbstracts)
        : fetchAwardedAbstracts(),
    ...
    if (po) {
      ...
      const prLines = abstract
        ? await fetchPRLinesForAbstract(abstract.pr_id)
        : [];
  ...
  const handleSave = async (status?: string) => {
    ...
    if (editingPO) {
      ...
      const effectiveStatus = status || editingPO.status || "DRAFT";
      if (effectiveStatus === "ISSUED") {
        const pendingSync = await upsertBudgetPendingDetailFromPO(
          editingPO.po_id,
        );
        if (!pendingSync.success) {
          console.error(
            "PO issued, but failed to sync Budget Pending Details:",
            pendingSync.error,
          );
        }
      }
      ...
      if (effectiveStatus === "ISSUED") {
        const pendingSync = await upsertBudgetPendingDetailFromPO(res.po_id);
        if (!pendingSync.success) {
          console.error(
            "PO issued, but failed to sync Budget Pending Details:",
            pendingSync.error,
          ...
          <div className="flex gap-2">
            ...
            {isReadOnly && (
              <>
                ...
                <button
                  onClick={() => {
                    const selectedAbstract = awardedAbstracts.find(
                      (a) => a.a_id === aId,
                    );
                    const winSupplier = aId ? supplierForAbstract(aId) : null;
                    ...
                    const mode = selectedAbstract
                      ? modes.find((m) => m.mp_id === selectedAbstract.mp_id)
                      : null;
          ...
          {!isReadOnly && (
            <div className="border-b-2 border-gray-800 px-3 py-3 bg-blue-50/60">
              ...
              >
                ...
                {awardedAbstracts
                  .filter((a) => {
                    // When creating new PO, hide abstracts already linked to an existing PO
                    if (editingPO && editingPO.a_id === a.a_id) return true;
                    return !orders.some((po) => po.a_id === a.a_id);
                  })
                  .map((a) => {
