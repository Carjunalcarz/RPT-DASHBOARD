...
const AbstractOfBids = () => {
  ...
  const loadList = useCallback(async () => {
    ...
    setCenters(rc);
    setSections(secs);
    setBudgetGlOptions(glOptions as PRBudgetGlAccountOption[]);
    setModes(mp);
    setDeliveryTerms(dt);
    setPaymentTerms(pt);
    setSuppliers(sups);
  ...
  const openForm = async (abs?: Abstract) => {
    const [sups, mp, dt, pt, prs, rc, secs, glOptions, prsWithWinners] =
      await Promise.all([
        fetchSuppliers(),
                ...
                <td className={`${cellBorder} ${cellPad} border-b-0`}>
                  ...
                  >
                    ...
                    {submittedPRs
                      ...
                      .map((pr) => {
                        const deptName = rcName(pr.rc_id);
                        const secName =
