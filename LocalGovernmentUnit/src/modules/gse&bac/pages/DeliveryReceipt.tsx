...
const DeliveryReceipt = () => {
  ...
  const handleSave = async (statusOverride?: string) => {
    ...
    const saveStatus = statusOverride || drStatus;
    ...
    if (hasPendingItems) {
      alert(
        "Cannot save: PO line items are missing.\n\nPlease go to Purchase Order page, open this PO (even though it shows RECEIVED), and click Save to create the line items in the database.",
      );
      return;
    }
    ...
    try {
      ...
      if (isNewDR) {
        ...
        if (selectedDR.status !== saveStatus) {
          await logWorkflowChange(
            "DR",
            drId,
            drNo,
  ...
  const handleAccept = async () => {
    ...
    const hasDeliveredItems = drLines.some((l) => l.qty_this_delivery > 0);
    if (!hasDeliveredItems) {
      alert("Please enter delivery quantities before accepting.");
      return;
    }

    if (
      !confirm(
        "Accept this delivery? This action cannot be undone and the receipt will become read-only.",
      )
    ) {
      return;
    }

    await handleSave("ACCEPTED");
  };
        ...
        <PageHeader
          ...
          subtitle={
            isNewDR
              ...
              : isReadonly
                ? "Viewing accepted delivery receipt (read-only)"
