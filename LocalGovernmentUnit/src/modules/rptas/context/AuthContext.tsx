...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    // Attempt to automatically get token from Supabase session if using Supabase Auth
    ...
    syncSupabaseSession().then(() => {
      ...
      if (authMode === 'mock') {
          ...
          api.get('/users/me')
            ...
            .catch(err => {
               console.warn('Failed to validate session from server, using cached user if available', err);
               // Instead of forcefully logging out on network error, keep the user logged in if they have cached data
