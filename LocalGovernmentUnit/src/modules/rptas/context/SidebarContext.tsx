...
import sidebarService, { SidebarItem } from '@/modules/rptas/shared/services/sidebarService';
...
export const SidebarProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const refreshMenu = async () => {
    ...
    try {
      ...
      // Attempting to load from API if needed, but since we are in the new app we can fallback to static/cached immediately 
      // or simply rely on cached DB for now without throwing unhandled network errors
      try {
        ...
        console.warn('API sidebar fetch failed, using cached data if available', apiError);
        const cachedDb = readCachedDbMenu();
        if (cachedDb.length) {
            setDbMenuItems(cachedDb);
            setMenuItems(cachedDb);
        }
      }
    } catch (error) {
      console.error('Failed to process sidebar items:', error);
    } finally {
