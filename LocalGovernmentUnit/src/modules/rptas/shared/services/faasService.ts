...
export const updateFaasStatus = async (
  id: string,
  status: 'draft' | 'for-review' | 'approved' | 'rejected' | 'pending-municipal' | 'pending-provincial',
  remarks?: string,
  meta?: { approverName?: string; approverPosition?: string }
): Promise<FaasRecord> => {
  try {
...
export const batchUpdateFaasStatus = async (
  ids: string[],
  status: 'draft' | 'for-review' | 'approved' | 'rejected' | 'pending-municipal' | 'pending-provincial',
  remarks?: string,
  meta?: { approverName?: string; approverPosition?: string }
