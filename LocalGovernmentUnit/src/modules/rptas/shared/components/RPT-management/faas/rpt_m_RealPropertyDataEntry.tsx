...
const RealPropertyDataEntry: React.FC = () => {
  ...
  const handleAutoSave = async (updatedAssessmentRecords: any[]) => {
    ...
    autoSaveTimerRef.current = window.setTimeout(async () => {
      ...
      const { id: _id, tdn, pin, arp, ...recordData } = currentRecord as any;
      const dataToSave = {
        ...recordData,
        ...
        <div className="flex flex-col gap-2">
          <div className="flex-1 min-w-0 flex items-center gap-2 flex-nowrap overflow-x-auto pb-2 pr-2">
            ...
            <button
              ...
              className="px-3 py-2 text-xs bg-white dark:bg-slate-700 hover:bg-primary/5 dark:hover:bg-primary/5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm transition-colors flex items-center gap-1.5 text-primary-light dark:text-primary/80 disabled:opacity-50 disabled:cursor-not-allowed h-9 whitespace-nowrap text-white dark:text-white"
            ...
            {(!isAdding && !isEditing) && (
              <button 
                ...
                disabled={!selectedRecord}
                className="px-3 py-2 text-xs bg-primary hover:bg-primary-light text-white dark:text-white rounded-lg shadow-sm transition-colors flex items-center gap-1.5 font-medium disabled:opacity-50 disabled:cursor-not-allowed h-9 whitespace-nowrap"
              >
                <FileText size={14} />
                Transaction
              </button>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
          ...
          >
            <table className="w-full text-sm" data-testid="records-table">
              <thead
                className="text-white sticky top-0 z-10 bg-primary"
              >
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {isLoading ? (
                  ...
                  <tr
                    ...
                    className={`cursor-pointer transition-colors ${
                      selectedRecord?.id === record.id
                        ...
                        : index % 2 === 0
                        ? 'bg-white dark:bg-slate-900 hover:bg-emerald-50 dark:hover:bg-emerald-900/10'
                        : 'bg-white dark:bg-slate-900 hover:bg-emerald-50 dark:hover:bg-emerald-900/10'
                    }`}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-mono text-slate-700 dark:text-slate-300 tracking-wider whitespace-nowrap text-center">{getDisplayTaxBegYear(record) || 'N/A'}</td>
          ...
          {/* Resize Handle */}
          <div
            className={`w-full h-5 bg-slate-50 dark:bg-slate-800 cursor-row-resize flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors select-none ${isResizing ? 'bg-primary/5 dark:bg-primary/5' : ''}`}
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-2 min-w-[200px]">
                ...
                <select
                  ...
                  className="flex-1 px-3 py-2 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50"
                ...
                <div className="flex-1 flex gap-2">
                  {searchField === 'TAX_BEG_YR' ? (
                    <select
                      ...
                      className={`flex-1 px-3 py-2 text-xs bg-slate-50 dark:bg-slate-800 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 ${
                  ...
                  <button 
                    ...
                    disabled={isLoading}
                    className="px-4 py-2 text-xs bg-primary hover:bg-primary-light text-white dark:text-white rounded-lg font-medium transition-colors whitespace-nowrap disabled:opacity-50 flex items-center gap-2"
                  >
