  ...
  const handleUpdate = (type: AssessmentType, updatedTypeRecords: any[]) => {
    ...
    const mappedUpdatedRecords = updatedTypeRecords.map(r => {
        ...
        return {
            ...
            CLASS_LEVEL: r.CLASS_LEVEL ?? r.classLevel,
            ORDINANCE_NO: r.ORDINANCE_NO ?? r.ordinanceNo,
            ORDINANCE_DATE_APPROVED: r.ORDINANCE_DATE_APPROVED ?? r.ordinanceDateApproved,
            AREA: r.AREA ?? r.area,
            IF_DEFAULT: r.IF_DEFAULT ?? r.ifDefault,
            ...
            TAXABLE_RATE: r.TAXABLE_RATE ?? (r.taxable ? 100 : 0),
        ...
        <button
          ...
          className={`flex-1 px-4 py-2.5 rounded-lg text-xs font-medium transition-all flex items-center justify-center gap-2 ${
            activeType === 'building'
              ...
              : 'bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-primary/5 dark:hover:bg-primary/5'
          }`}
