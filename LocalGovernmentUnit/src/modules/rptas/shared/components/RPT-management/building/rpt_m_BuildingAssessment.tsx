  ...
  useEffect(() => {
    if (apiRecords) {
      const mapped = apiRecords.map((r, index) => ({
        ...
        adjustedMarketValue: (r.STRUCTURE_MARKET_VAL ?? r.MARKET_VAL) || 0, // Net value (after depreciation / additional items)
  ...
  const handleStructureUpdate = (updatedStructures: BldgStrucRecord[]) => {
    ...
    if (updatedStructures.length > 0) {
      ...
      setFormData(prev => ({
        ...
        actualUse: (structure as any).Actual_use || structure.Struc_type || prev.actualUse,
      ...
      if (selectedRecord) {
        ...
        const updatedRecord = {
          ...
          actualUse: (structure as any).Actual_use || structure.Struc_type || selectedRecord.actualUse,
