...
const BuildingStructureModal: React.FC<BuildingStructureModalProps> = ({ 
  ...
  const handleRowSelect = (record: BldgStrucRecord) => {
    ...
    setFormData({
      classification: pickString((record as any).Classification, (record as any).CLASSIFICATION, parentClassification),
      structureType: pickString((record as any).Actual_use, (record as any).ACTUAL_USE, (record as any).Struc_type, (record as any).STRUC_TYPE, parentStructureType),
      subClass: pickString((record as any).SubClass, (record as any).SUB_CLASS, (record as any).Sub_Class, parentSubType),
      bldgCode: record.BldgCode || '',
      storey: record.Storey || '',
      floorOrder: record.FloorOrd?.toString() || '0',
      description: record.Struc_Desc || '',
      age: record.Age || '',
      floorArea: (record.Floor_area ?? record.Total_Area)?.toString() || parentArea || '0',
      unitValue: record.UNIT_VALUE?.toString() || parentUnitValue || '0',
  ...
  const handleDelete = async () => {
    ...
    const isConfirmed = await showConfirm({
      ...
      message: 'Delete this structure record?',
  ...
  const handleSave = async () => {
    try {
      const recordData: Partial<BldgStrucRecord> = {
        ...
        Actual_use: formData.structureType,
          ...
          <div className="flex gap-1">
            ...
            {isLocalFormEnabled && (
              <>
                <button
                  onClick={handleSave}
                  className="px-2 py-1 text-xs bg-primary hover:bg-primary-light text-white dark:text-white rounded shadow-sm transition-colors flex items-center gap-1"
                >
                  <Save size={12} />
                  Save
                </button>
