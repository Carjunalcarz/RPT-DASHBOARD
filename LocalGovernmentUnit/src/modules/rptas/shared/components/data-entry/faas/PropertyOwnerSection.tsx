...
const PropertyOwnerSection: React.FC<PropertyOwnerSectionProps> = ({ isEnabled, selectedRecord, onUpdate }) => {
  ...
  const handleOwnerChange = (field: keyof OwnerData, value: string) => {
    setData(prev => {
        ...
        if (onUpdate) {
            ...
            if (field === 'number') updatePayload.ownerNo = value;
  ...
  const selectClass = `w-full px-3 py-2 text-xs bg-surface dark:bg-background border border-border dark:border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all ${
    !isEnabled ? 'bg-background dark:bg-background/50 cursor-not-allowed opacity-70' : ''
  }`;

  return (
    <div className="bg-gradient-to-br from-slate-50 to-green-50/30 dark:from-slate-800/50 dark:to-green-900/10 rounded-xl border border-border dark:border-border overflow-hidden">
      {/* Section Header */}
      <div
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-12 gap-2 mb-2 px-2">
          ...
          <div className="col-span-2 text-xs font-semibold text-muted dark:text-muted">Code</div>
          <div className="col-span-2 text-xs font-semibold text-muted dark:text-muted">Number</div>
          <div className="col-span-3 text-xs font-semibold text-muted dark:text-muted">Name</div>
          <div className="col-span-3 text-xs font-semibold text-muted dark:text-muted">Address</div>
        </div>

        {/* Owner Row */}
        <div className="grid grid-cols-12 gap-2 items-center mb-3 p-2 bg-surface dark:bg-background/50 rounded-lg border border-border dark:border-border">
          <div className="col-span-2 flex items-center gap-2">
            <button
              disabled={!isEnabled}
              className="p-1.5 hover:bg-muted/10 dark:hover:bg-muted/20 rounded-lg transition-colors disabled:opacity-50"
              title="Search Owner"
              data-testid="btn-search-owner"
            >
              <Search size={14} className="text-primary dark:text-primary" />
            </button>
            <span className="text-xs font-semibold text-foreground dark:text-foreground">Owner</span>
          </div>
        ...
        {/* Administrator Row */}
        <div className="grid grid-cols-12 gap-2 items-center p-2 bg-surface dark:bg-background/50 rounded-lg border border-border dark:border-border">
          <div className="col-span-2 flex items-center gap-2">
            <button
              disabled={!isEnabled}
              className="p-1.5 hover:bg-muted/10 dark:hover:bg-muted/20 rounded-lg transition-colors disabled:opacity-50"
            ...
            <span className="text-xs font-semibold text-foreground dark:text-foreground">Administrator</span>
          </div>
          <div className="col-span-2">
            <select
