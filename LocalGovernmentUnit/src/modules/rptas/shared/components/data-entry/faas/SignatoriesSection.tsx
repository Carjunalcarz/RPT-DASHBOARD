  ...
  React.useEffect(() => {
    if (initialRecord) {
      setFormData({
        ...
        checkedBy: initialRecord.provincialApprover || initialRecord.municipalApprover || '',
        checkedPosition: (initialRecord.provincialApprover || initialRecord.municipalApprover) ? 'Approval Account' : '',
        checkedDate: (initialRecord.provincialApprovalDate || initialRecord.municipalApprovalDate)
          ? (initialRecord.provincialApprovalDate || initialRecord.municipalApprovalDate).split('T')[0]
          : '',
        checkedSGD: false,
        checkedTPD: false,
        ...
        entryDate: (initialRecord.provincialApprovalDate || initialRecord.municipalApprovalDate)
          ? (initialRecord.provincialApprovalDate || initialRecord.municipalApprovalDate).split('T')[0]
          : '',
        entryBy: initialRecord.provincialApprover || initialRecord.municipalApprover || '',
      });
    }
  }, [initialRecord]);
      ...
      <div className="p-3">
        ...
        {activeSubTab === 'memorandum' && (
          <div className="space-y-3">
            ...
            {isSavingMemo && (
              <div className="rounded-lg border border-primary/20 bg-primary/5 px-3 py-2 text-xs text-primary-light dark:border-primary-light/40 dark:bg-primary/5 dark:text-primary/40 text-white dark:text-white">
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    <div className="relative flex-1 min-w-[180px]">
                      ...
                      <input
                        ...
                        className="w-full pl-7 pr-3 py-2 text-xs bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/50"
                      />
                    </div>
                    <select
                    ...
                    {selectedMemoId && (
                      <button
                        ...
                        className="px-3 py-2 text-xs bg-primary hover:bg-primary-light text-white dark:text-white rounded-lg flex items-center gap-2"
                      >
                        <Mail size={14} />
                        Notify
                      </button>
                          ...
                          filteredMemorandums.map((item) => (
                            <tr
                              ...
                              className={`border-b border-slate-200 dark:border-slate-700 cursor-pointer ${
                                selectedMemoId === item.id ? 'bg-primary/5 dark:bg-primary/5' : 'hover:bg-slate-50 dark:hover:bg-slate-800/60'
                              }`}
                            >
                        ...
                        {filteredSwornStatements.length === 0 ? (
                          ...
                          filteredSwornStatements.map((item) => (
                            <tr
                              ...
                              className={`border-b border-slate-200 dark:border-slate-700 cursor-pointer ${
                                selectedSwornId === item.id ? 'bg-primary/5 dark:bg-primary/5' : 'hover:bg-slate-50 dark:hover:bg-slate-800/60'
                              }`}
                            >
                              <td className="px-3 py-2 text-slate-700 dark:text-slate-200">
                                {item.signatory}
          ...
          <div className="space-y-3">
            <div>
              <textarea
                ...
                className="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded focus:outline-none focus:ring-1 focus:ring-primary/50 disabled:opacity-60"
              />
            </div>
          </div>
        )}
