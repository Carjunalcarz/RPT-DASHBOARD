...
import { useMainClassOptions } from '../useMainClassOptions';
import ActualUseSelect from '../ActualUseSelect';
import { useActualUseOptions } from '../useActualUseOptions';
...
const LandAssessment: React.FC<LandAssessmentProps> = ({ dataSource = 'mssql', records: apiRecords, isEnabled, tdn, onUpdate, onPrint }) => {
  ...
  const { options: mainClassOptions, codes: mainClassCodes, isLoading: isLoadingMainClasses } = useMainClassOptions();
  const { options: actualUseOptions, codes: actualUseCodes, isLoading: isLoadingActualUses } = useActualUseOptions(formData.classification);
  ...
  */
  ...
  useEffect(() => {
    ...
    if (
      ...
      !isAdding &&
      selectedRecord &&
      String(formData.classification || '').trim().toUpperCase() === String(selectedRecord.classification || '').trim().toUpperCase() &&
      String(formData.actualUse || '').trim().toUpperCase() === String(selectedRecord.actualUse || '').trim().toUpperCase() &&
      Boolean(formData.ifDefault) === Boolean(selectedRecord.ifDefault)
    ) {
      return;
    }
  ...
  }, [formData.classification, formData.actualUse, formData.ifDefault, isAdding, isEditing, selectedRecord]);
  ...
  useEffect(() => {
    ...
    if (apiRecords && apiRecords.length > 0) {
      ...
      const mapped = apiRecords.map((r, index) => {
        const id =
          r.id ||
          `${r.TDN}-L-${String(r.CLASSIFICATION || '').trim()}-${String(r.ACTUAL_USE || '').trim()}-${String(r.SUB_CLASS || '').trim()}-${index}`;
        return {
          id,
  ...
  const handleAdd = () => {
    ...
    const baseTdn = String(tdn || selectedTdn || apiRecords?.[0]?.TDN || '').trim();
    if (baseTdn) setSelectedTdn(baseTdn);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const baseTdn = String(tdn || selectedTdn || selectedRecord?.tdn || apiRecords?.[0]?.TDN || '').trim();
      if (!baseTdn) {
      ...
      const newRecord: LandRecord = {
        id: isAdding
          ? `${baseTdn}-L-${String(formData.classification || '').trim()}-${String(formData.actualUse || '').trim()}-${String(formData.subClass || '').trim()}-${Date.now()}`
          : selectedRecord!.id,
        uniqueId: selectedRecord?.uniqueId || `${baseTdn}-L-${String(formData.classification || '').trim()}-${String(formData.actualUse || '').trim()}-${String(formData.subClass || '').trim()}-${Date.now()}`,
        tdn: baseTdn,
        kind: 'Land',
