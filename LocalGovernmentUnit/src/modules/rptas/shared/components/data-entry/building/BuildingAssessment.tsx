...
const BuildingAssessment: React.FC<BuildingAssessmentProps> = ({
  ...
  const lastApiSignatureRef = useRef('');
  ...
  useEffect(() => {
    if (apiRecords) {
      const signature = apiRecords
        ...
        .join('~');
      if (signature === lastApiSignatureRef.current) return;

      const mapped = apiRecords.map((r, index) => ({
        id:
          r.id ||
          `${r.TDN}-B-${String(r.CLASSIFICATION || '').trim()}-${String(r.ACTUAL_USE || '').trim()}-${String(r.SUB_CLASS || '').trim()}-${index}`,
        uniqueId:
          r.uniqueId ||
          r.id ||
          `${r.TDN}-B-${String(r.CLASSIFICATION || '').trim()}-${String(r.ACTUAL_USE || '').trim()}-${String(r.SUB_CLASS || '').trim()}-${index}`,
      ...
      lastApiSignatureRef.current = signature;
    } else {
      if (lastApiSignatureRef.current) {
        lastApiSignatureRef.current = '';
        setRecords([]);
      }
    }
  }, [apiRecords]);
  ...
  useEffect(() => {
    ...
    const mapped: RptAssRecord[] = records.map((r) => {
      const taxable = Boolean(r.taxable);
      const resolvedTdn = String(r.tdn || tdn || '').trim();
      return {
        TDN: resolvedTdn,
        KIND: 'B',
        CLASSIFICATION: String(r.classification || ''),
        ACTUAL_USE: String(r.actualUse || ''),
        SUB_CLASS: 'NONE',
        AREA: Number(r.area || 0),
        UNIT_VALUE: Number(r.unitValue || 0),
        MARKET_VAL: Number(r.baseMarketValue || 0),
        ASS_LEVEL: Number(r.assessmentLevel || 0),
        ASS_VALUE: Number(r.assessedValue || 0),
        TAXABILITY: taxable ? 'TAXABLE' : 'EXEMPT',
        TAXABLE_RATE: taxable ? 100 : 0,
        BU: r.beneficialUse ? '1' : '0',
        IdleLand: Boolean(r.idleLand),
      } as any;
    });

    if (mapped.some((r) => !String(r.TDN || '').trim())) return;
  ...
  // Update Unit Value when Classification / Structural Type changes
  useEffect(() => {
    if (!formData.classification || !formData.actualUse) return;
    if (appraisals.length === 0) return;

    const desiredSub = String(formData.subClass || 'NONE').trim() || 'NONE';
    const match =
      appraisals.find(
        (a) =>
          a.buildingType.trim() === formData.actualUse.trim() &&
          ((a.buildingSubClass || 'NONE').trim() === desiredSub)
      ) ||
      appraisals.find(
        (a) =>
          a.buildingType.trim() === formData.actualUse.trim() &&
          ((a.buildingSubClass || 'NONE').trim() === 'NONE')
      ) ||
      appraisals.find((a) => a.buildingType.trim() === formData.actualUse.trim());
  ...
  const handleStructureUpdate = (updatedStructures: BldgStrucRecord[]) => {
    ...
    if (updatedStructures.length > 0) {
      ...
      setFormData((prev) => ({
        ...prev,
        classification: prev.classification || selectedRecord?.classification || '',
        actualUse: prev.actualUse || selectedRecord?.actualUse || '',
        subClass: 'NONE',
  ...
  useEffect(() => {
    if (selectedRecord && selectedRecord.tdn) {
      ...
      getBldgStrucByTdn(selectedRecord.tdn)
        .then(data => {
          ...
          const shouldMatchSubClass = targetSubClass && targetSubClass !== 'NONE';
          ...
          getBldgAdjByTdn(selectedRecord.tdn)
            .then(adjData => {
  ...
  }, [selectedRecord?.tdn, selectedRecord?.classification, selectedRecord?.actualUse, selectedRecord?.subClass]);
  ...
  const handleRowSelect = (record: BuildingRecord) => {
    ...
    setFormData({
      ...
      actualUse: String(record.actualUse || '').trim(),
      subClass: 'NONE',
  ...
  useEffect(() => {
    ...
    setFormData({
      classification: String(first.classification || '').trim(),
      actualUse: String(first.actualUse || '').trim(),
      subClass: 'NONE',
  ...
  // Handle Save
  const handleSave = () => {
    const baseTdn = String(tdn || selectedRecord?.tdn || apiRecords?.[0]?.TDN || '').trim();
    if (!baseTdn) {
      toast.error('No TDN selected for this assessment');
      return;
    }

    const newRecord: BuildingRecord = {
      id: isAdding
        ? `${baseTdn}-B-${String(formData.classification || '').trim()}-${String(formData.actualUse || '').trim()}-${String(formData.subClass || '').trim()}-${Date.now()}`
        : selectedRecord!.id,
      uniqueId: isAdding
        ? `${baseTdn}-B-${String(formData.classification || '').trim()}-${String(formData.actualUse || '').trim()}-${String(formData.subClass || '').trim()}-${Date.now()}`
        : selectedRecord!.uniqueId,
      tdn: isAdding ? baseTdn : selectedRecord!.tdn,
      kind: 'Building',
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-800" data-testid="building-assessment">
                ...
                <tr
                  ...
                  className={`cursor-pointer transition-colors ${
                    selectedRecord?.id === record.id
                      ...
                      : index % 2 === 0
                      ? 'bg-primary/5 dark:bg-slate-800/50'
      ...
      <BuildingStructureModal
        ...
        buildingId={selectedRecord?.tdn || tdn || ''}
        isFormEnabled={isFormEnabled}
        initialStructures={structures}
        onUpdate={handleStructureUpdate}
        parentClassification={selectedRecord?.classification || formData.classification}
        parentStructureType={selectedRecord?.actualUse || formData.actualUse}
        parentSubType={'NONE'}
            ...
            <div className="flex items-center gap-2 relative">
              ...
              <div className="flex-1 relative min-w-0">
                <select
                  ...
                  className={`w-full h-8 text-xs bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded px-2 ${!isFormEnabled ? 'opacity-60 cursor-not-allowed' : ''}`}
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 relative min-w-0">
                <select
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, actualUse: e.target.value, subClass: 'NONE' }))
                  }
                  disabled={!isFormEnabled}
                  className={`w-full h-8 text-xs bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded px-2 ${!isFormEnabled ? 'opacity-60 cursor-not-allowed' : ''}`}
