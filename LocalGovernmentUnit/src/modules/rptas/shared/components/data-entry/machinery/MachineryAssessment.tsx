...
const MachineryAssessment: React.FC<MachineryAssessmentProps> = ({ records: apiRecords, isEnabled, tdn, onUpdate, onPrint }) => {
  ...
  useEffect(() => {
    ...
    const mapped: RptAssRecord[] = records.map((r) => {
      ...
      const recordTdn = String(r.tdn || tdn || '').trim();
      return {
        TDN: recordTdn,
        KIND: 'M',
        CLASSIFICATION: String(r.classification || ''),
        ACTUAL_USE: String(r.actualUse || ''),
        SUB_CLASS: String(r.subClass || ''),
  ...
  useEffect(() => {
    if (apiRecords) {
      ...
      const mapped = apiRecords
        .filter(
          (r) =>
            String(r.KIND || '').toUpperCase() === 'MACHINERY' ||
            String(r.KIND || '').toUpperCase() === 'MACH' ||
            String(r.KIND || '').toUpperCase() === 'M'
        )
        .map((r, index) => {
        const id =
          r.id ||
          `${r.TDN}-M-${String(r.CLASSIFICATION || '').trim()}-${String(r.ACTUAL_USE || '').trim()}-${String(r.SUB_CLASS || '').trim()}-${index}`;
        return {
        id,
        uniqueId: r.uniqueId || id,
        tdn: r.TDN,
        kind: 'Machinery',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!baseTdn) {
        toast.error('No TDN selected for this assessment', { id: toastId });
        return;
      }
      ...
      const newRecord: MachineryRecord = {
        id: isAdding
          ? `${baseTdn}-M-${String(formData.classification || '').trim()}-${String(formData.actualUse || '').trim()}-${String(formData.subClass || '').trim()}-${Date.now()}`
          : selectedRecord!.id,
        uniqueId:
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
            {records.length === 0 ? (
              ...
              records.map((record, index) => (
                <tr key={record.uniqueId || record.id} onClick={() => handleRowSelect(record)} className={`cursor-pointer hover:bg-primary/5 dark:hover:bg-slate-800 ${selectedRecord?.id === record.id ? 'bg-primary/10 dark:bg-primary/10' : index % 2 === 0 ? 'bg-slate-50 dark:bg-slate-800/50' : 'bg-white dark:bg-slate-900'}`}>
