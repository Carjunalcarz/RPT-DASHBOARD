...
import { 
  ...
  CreateTemplateDTO
} from '@/modules/rptas/shared/services/setupSignatoryTemplatesService';
...
import {
  ...
  SelectTrigger,
  SelectValue,
} from '@/modules/rptas/ui/select';
import { Label } from '@/modules/rptas/ui/label';
import { Switch } from '@/modules/rptas/ui/switch';
    ...
    return (
      <div className="grid grid-cols-12 gap-2 items-center py-1">
        ...
        <div className="col-span-3">
          <Label className="block text-[10px] font-medium text-muted dark:text-muted mb-0.5 uppercase">{label}:</Label>
          <Select
            value={formData[idField] as string}
            onValueChange={(value) => setFormData({ ...formData, [idField]: value })}
          >
            <SelectTrigger className="h-7 text-xs bg-background dark:bg-background border-border dark:border-border">
        ...
        <div className="col-span-4">
          <Label className="block text-[10px] font-medium text-muted dark:text-muted mb-0.5 uppercase">Position:</Label>
          <Input
            type="text"
            value={selectedSignatory?.title || ''}
            readOnly
            className="h-7 text-xs bg-muted/10 dark:bg-surface border-border dark:border-border disabled:opacity-60"
            placeholder="Position will appear here"
          />
        </div>
        <div className="col-span-3">
          <Label className="block text-[10px] font-medium text-muted dark:text-muted mb-0.5 uppercase">Date:</Label>
          <Input
            type="date"
            value={formData[dateField] as string}
            onChange={(e) => setFormData({ ...formData, [dateField]: e.target.value })}
            className="h-7 text-xs bg-background dark:bg-background border-border dark:border-border"
          />
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-foreground dark:text-surface">Signatory Templates</h2>
          <p className="text-sm text-muted dark:text-muted">Configure reusable signatory sets by year.</p>
        </div>
      ...
      <Card className="border-border dark:border-border shadow-sm">
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-background dark:bg-surface/50">
              <TableRow>
            ...
            <TableBody>
              {loading ? (
                ...
                templates.map((template) => (
                  <TableRow key={template.id} className="hover:bg-muted/5/50 dark:hover:bg-muted/10/50 transition-colors">
                    <TableCell className="font-medium">{template.year}</TableCell>
                    <TableCell>{template.description || '-'}</TableCell>
                    <TableCell>
                      <div className="flex flex-col gap-1 text-xs text-muted dark:text-muted">
                        {template.approvedBy && (
                    ...
                    <TableCell className="text-center">
                      {template.isActive ? (
                        ...
                        <Badge variant="outline" className="bg-muted/10 text-muted border-border dark:bg-background dark:text-muted dark:border-border">
                          Inactive
                        </Badge>
                      )}
                    </TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem 
                            ...
                            className="text-danger dark:text-danger focus:text-danger"
                          >
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle className="text-danger">Delete Template</DialogTitle>
            <DialogDescription>
