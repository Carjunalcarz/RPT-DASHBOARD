...
const RptBreadcrumb: React.FC<RptBreadcrumbProps> = ({ items, className = '' }) => {
  return (
    <nav 
      className={`flex items-center gap-2 text-xs text-blue-100 mb-1 ${className}`} 
      aria-label="Breadcrumb"
      data-testid="rpt-breadcrumb"
    >
      <Link
        to="/"
        className="flex items-center gap-1 hover:text-surface transition-colors"
        ...
        return (
          <React.Fragment key={`${item.label}-${index}`}>
            ...
            {item.path && !isLast ? (
              <Link
                to={item.path}
                className="hover:text-surface transition-colors flex items-center gap-1"
              ...
              <span 
                className="text-surface font-medium flex items-center gap-1"
