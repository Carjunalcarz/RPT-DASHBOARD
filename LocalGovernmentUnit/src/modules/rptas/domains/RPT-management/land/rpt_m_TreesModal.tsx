    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl" aria-describedby={undefined}>
        ...
        <div className="space-y-4 py-4">
          <div className="flex gap-2 items-end bg-background dark:bg-background/50 p-3 rounded border border-border dark:border-border">
            <div className="flex-1">
              ...
              <select
                ...
                disabled={readOnly}
                className="w-full px-2 py-1.5 text-xs border rounded bg-surface dark:bg-surface border-border dark:border-border disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            <div className="w-32">
              ...
              <input
                ...
                disabled={readOnly}
                placeholder="0.00"
                className="w-full px-2 py-1.5 text-xs border rounded bg-surface dark:bg-surface border-border dark:border-border text-right disabled:opacity-50 disabled:cursor-not-allowed"
              />
            </div>
            <div className="w-24">
              <label className="text-xs font-medium text-foreground dark:text-foreground mb-1 block">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-success hover:bg-success/90 text-surface rounded text-xs flex items-center gap-1 disabled:opacity-50"
            >
              <Plus size={14} />
              Add
            </button>
          ...
          <div className="border border-border dark:border-border rounded-lg overflow-hidden max-h-[300px] overflow-y-auto">
            <table className="w-full text-xs">
              <thead className="bg-muted/10 dark:bg-background text-muted dark:text-foreground sticky top-0">
                <tr>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {trees.length === 0 ? (
                  ...
                  trees.map((tree) => (
                    <tr key={tree.id} className="bg-surface dark:bg-surface">
                      <td className="px-3 py-2">{tree.name}</td>
                      <td className="px-3 py-2">
                        <span className={`px-1.5 py-0.5 rounded text-[10px] ${tree.type === 'FB' ? 'bg-success/20 text-success' : 'bg-warning/20 text-warning'}`}>
              ...
              <tfoot className="bg-background dark:bg-background font-semibold border-t border-border dark:border-border sticky bottom-0">
                <tr>
        ...
        <DialogFooter>
          ...
          {!readOnly && (
            <button
              onClick={handleSave}
              className="px-4 py-2 text-xs bg-success hover:bg-success/90 text-surface rounded transition-colors flex items-center gap-2"
            >
              <Save size={14} />
              Save Plants/Trees
            </button>
