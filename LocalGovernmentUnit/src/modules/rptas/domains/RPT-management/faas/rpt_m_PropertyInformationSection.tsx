...
const PropertyInformationSection: React.FC<PropertyInformationSectionProps> = ({
  ...
  // Tracks which record the form has already been initialized from, so we only
  // rebuild `data` when the record identity actually changes — not on every new
  // selectedRecord reference (the parent re-creates it on each SWR refresh) or
  // when municipalityOptions resolve. Rebuilding on those caused a setData storm
  // that re-triggered the barangay fetch + name effects and froze this tab.
  const appliedRecordKeyRef = useRef<string | null | undefined>(undefined);
  ...
  useEffect(() => {
    // Only re-initialize when the selected record's identity changes.
    const recordKey = selectedRecord ? (selectedRecord.id ?? 'no-id') : null;
    if (appliedRecordKeyRef.current === recordKey) return;
    appliedRecordKeyRef.current = recordKey;
