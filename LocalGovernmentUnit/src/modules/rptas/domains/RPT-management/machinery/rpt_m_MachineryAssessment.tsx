...
const MachineryAssessment: React.FC<MachineryAssessmentProps> = ({ 
  ...
  return (
    <div className="bg-surface dark:bg-surface rounded-lg shadow-sm border border-border dark:border-border" data-testid="machinery-assessment">
      {/* Header */}
      <div
        className="px-4 py-3 rounded-t-lg bg-primary"
      >
        <div className="flex items-center gap-2">
          <Settings size={20} className="text-surface" />
          <h2 className="text-base font-semibold text-surface">Real Property Assessment (Machinery)</h2>
        </div>
      </div>
      ...
      <div className="bg-transparent border-b border-border dark:border-border px-3 py-2">
        <div className="flex flex-wrap gap-1">
          <button onClick={handleAdd} disabled={!canModify} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1.5 disabled:opacity-50">
            <Plus size={14} /> Add
          </button>
          <button onClick={handleEdit} disabled={!selectedRecord || !canModify} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1.5 disabled:opacity-50">
            <Edit2 size={14} /> Edit
          </button>
          <button onClick={handleDelete} disabled={!selectedRecord || !canModify} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-danger/10 border rounded shadow-sm flex items-center gap-1.5 text-danger disabled:opacity-50">
            <Trash2 size={14} /> Delete
          </button>
          <div className="w-px h-6 bg-slate-300 mx-1 self-center" />
          <button onClick={handleSave} disabled={!isLocalFormEnabled} className="px-3 py-1.5 text-xs bg-primary text-surface hover:bg-primary-light rounded shadow-sm flex items-center gap-1.5 disabled:opacity-50">
            <Save size={14} /> Save
          </button>
          <button onClick={handleCancel} disabled={!isLocalFormEnabled} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1.5 disabled:opacity-50">
            <X size={14} /> Cancel
          </button>
          <div className="w-px h-6 bg-slate-300 mx-1 self-center" />
          <button onClick={handleRefresh} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1.5">
            <RefreshCw size={14} /> Refresh
          </button>
          <button onClick={() => setIsAdjustmentOpen(true)} disabled={!selectedRecord} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1.5 text-warning disabled:opacity-50">
            <ArrowDownUp size={14} /> Adjustment
          </button>
          <button
            onClick={handlePrint}
            disabled={status !== 'approved'}
            className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead className="text-surface bg-primary">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
            {records.length === 0 ? (
              <tr><td colSpan={11} className="px-4 py-8 text-center text-muted">No machinery records found.</td></tr>
            ) : (
              records.map((record, index) => (
                <tr key={record.uniqueId || record.id} onClick={() => handleRowSelect(record)} className={`cursor-pointer hover:bg-primary/10 dark:hover:bg-muted/10 ${selectedRecord?.id === record.id ? 'bg-primary/20 dark:bg-primary/30' : index % 2 === 0 ? 'bg-background dark:bg-background/50' : 'bg-surface dark:bg-surface'}`}>
                  <td className="px-2 py-1.5 border-r border-border dark:border-border">{record.kind}</td>
                  ...
                  <td className="px-2 py-1.5 text-right border-r border-border dark:border-border">{record.area}</td>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            {isStandardRevision ? (
              <div className="rounded-lg border border-border bg-surface dark:bg-muted/20 px-3 py-2 text-[11px] text-muted dark:text-muted">
                Revision: Only Unit Value and Assessment Level are editable. Classification and use fields are read-only (MSSQL).
              </div>
            ) : null}
            <div className="flex items-center gap-2">
              <label className="w-28 text-xs font-medium text-right">Classification:</label>
              <select value={formData.classification} onChange={(e) => setFormData(prev => ({ ...prev, classification: e.target.value }))} disabled={!isFormEnabled || isStandardRevision} className="flex-1 px-2 py-1.5 text-xs border rounded">
            ...
            <div className="flex items-center gap-2">
              ...
              <select value={formData.subClass} onChange={(e) => setFormData(prev => ({ ...prev, subClass: e.target.value }))} disabled={!isFormEnabled || isStandardRevision} className="flex-1 px-2 py-1.5 text-xs border rounded">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 flex items-center gap-1">
                <input type="number" value={formData.area} onChange={(e) => setFormData(prev => ({ ...prev, area: e.target.value }))} disabled={!isFormEnabled || isStandardRevision} className="flex-1 px-2 py-1.5 text-xs text-right border rounded" />
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <input type="text" value={formatCurrency(computedValues.baseMarketValue)} readOnly className="flex-1 px-2 py-1.5 text-xs text-right bg-muted/10 border rounded" />
            </div>
            <div className="flex items-center gap-2">
              <label className="w-36 text-xs font-medium text-right">Adjusted Market Value:</label>
              <input type="text" value={formatCurrency(computedValues.adjustedMarketValue)} readOnly className="flex-1 px-2 py-1.5 text-xs text-right bg-muted/10 border rounded" />
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <input type="text" value={formatCurrency(computedValues.assessedValue)} readOnly className="flex-1 px-2 py-1.5 text-xs text-right bg-muted/10 border rounded font-medium" />
            </div>
            <div className="flex items-center gap-2">
              <label className="w-28 text-xs font-medium text-right">Taxable:</label>
              <input type="checkbox" checked={formData.taxable} onChange={(e) => setFormData(prev => ({ ...prev, taxable: e.target.checked }))} disabled={!isFormEnabled || isStandardRevision} className="w-4 h-4" />
            </div>
            <div className="flex items-center gap-2">
              <label className="w-28 text-xs font-medium text-right">Beneficial Use:</label>
              <input type="checkbox" checked={formData.beneficialUse} onChange={(e) => setFormData(prev => ({ ...prev, beneficialUse: e.target.checked }))} disabled={!isFormEnabled || isStandardRevision} className="w-4 h-4" />
            </div>
            <div className="flex items-center gap-2">
              <label className="w-28 text-xs font-medium text-right">Idle Land:</label>
              <input type="checkbox" checked={formData.idleLand} onChange={(e) => setFormData(prev => ({ ...prev, idleLand: e.target.checked }))} disabled={!isFormEnabled || isStandardRevision} className="w-4 h-4" />
            </div>
          </div>
        </div>
      </div>
