    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          {/* Toolbar */}
          <div className="flex flex-wrap gap-1 bg-muted/10 dark:bg-background p-2 rounded border border-border dark:border-border">
            <button onClick={handleAdd} disabled={isLocalFormEnabled} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1 disabled:opacity-50">
              <Plus size={14} /> Add
            </button>
            <button onClick={handleEdit} disabled={!selectedRecord || isLocalFormEnabled} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1 disabled:opacity-50">
              <Edit2 size={14} /> Edit
            </button>
            <button onClick={handleDelete} disabled={!selectedRecord || isLocalFormEnabled} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-danger/10 border rounded shadow-sm flex items-center gap-1 text-danger disabled:opacity-50">
              <Trash2 size={14} /> Delete
            </button>
            <div className="w-px h-6 bg-slate-300 mx-1 self-center" />
            <button onClick={handleSave} disabled={!isLocalFormEnabled} className="px-3 py-1.5 text-xs bg-primary text-surface hover:bg-primary-light rounded shadow-sm flex items-center gap-1 disabled:opacity-50">
              <Save size={14} /> Save
            </button>
            <button onClick={handleCancel} disabled={!isLocalFormEnabled} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1 disabled:opacity-50">
              <X size={14} /> Cancel
            </button>
            <div className="w-px h-6 bg-slate-300 mx-1 self-center" />
            <button onClick={fetchRecords} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1">
              <RefreshCw size={14} /> Refresh
            </button>
            <button onClick={handleCompute} disabled={!isLocalFormEnabled} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1 disabled:opacity-50">
              <Calculator size={14} /> Compute
            </button>
            <button onClick={() => onOpenChange(false)} className="px-3 py-1.5 text-xs bg-surface dark:bg-muted/20 hover:bg-muted/5 border rounded shadow-sm flex items-center gap-1 ml-auto">
              Close
            </button>
          </div>

          {/* Table */}
          <div className="border border-border dark:border-border rounded h-48 overflow-auto">
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-muted/10 dark:bg-background text-foreground dark:text-foreground">
                <tr>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {records.length === 0 ? (
                    <tr><td colSpan={10} className="px-2 py-4 text-center text-muted">No machinery records found.</td></tr>
                ) : (
                    records.map((r, i) => (
                        <tr key={i} onClick={() => handleRowSelect(r)} className={`cursor-pointer hover:bg-primary/10 dark:hover:bg-muted/10 ${selectedRecord === r ? 'bg-primary/20 dark:bg-primary/30' : ''}`}>
          ...
          {/* Detail Form */}
          <div className={`bg-muted/10 dark:bg-background/50 p-3 rounded border border-border dark:border-border ${!isLocalFormEnabled ? 'opacity-90 pointer-events-none' : ''}`}>
        ...
        <DialogFooter>
          <button onClick={() => onOpenChange(false)} className="px-4 py-2 text-xs border border-border rounded hover:bg-muted/5">Close</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
