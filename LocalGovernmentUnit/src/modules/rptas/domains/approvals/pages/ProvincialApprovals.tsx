...
const ProvincialApprovals: React.FC = () => {
  const navigate = useNavigate();
  const { headerColor, headerColorDark } = useThemeColor();
  const { showConfirm } = useAlert();
  const PROVINCIAL_ASSESSOR_DEPARTMENT = 'Provincial Assessor Office';
  const SIGNATORY_STORAGE_KEY = 'rptas.provincialApproval.signatoryId';
  
  const [pagination, setPagination] = useState({ page: 1, limit: 10, totalPages: 1 });
  const statusFilter = 'pending-provincial';
  ...
  const canApprove = Boolean(signatoryMeta.approverName);

  // Fetch records based on selected status
  const { data, error, isLoading, mutate } = useSWR(
    ['pending-provincial-approvals', pagination.page, pagination.limit],
    ([, page, limit]) => {
  ...
  const handleBulkApprove = async () => {
    ...
    if (!canApprove) {
      toast.error('Please select a Provincial Assessor signatory first.');
      return;
    }

    const actionLabel = 'Approve';

    const confirmed = await showConfirm({
      title: `Bulk Approve`,
      message: `Are you sure you want to approve ${selectedIds.size} selected properties? This will finalize the record.`,
  ...
  const pageTitle = 'Provincial Approvals';
  const pageSubtitle = 'Final review and approval of properties.';
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border bg-accent/10 text-accent dark:bg-accent/20 dark:text-accent-light border-accent/20 dark:border-accent/40">
                          <CheckCircle size={12} />
                          Provincial Review
                        </span>
                      </td>
...
export default ProvincialApprovals;
