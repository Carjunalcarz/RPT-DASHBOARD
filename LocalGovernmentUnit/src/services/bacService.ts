...
export const fetchPurchaseOrderDetail = async (
  ...
  const { data: poRow, error: poError } = await client
    .schema("bac")
    .from("purchase_order")
    .select("po_id, po_no, po_total_amount, a_id")
    .eq("po_id", poId)
    .maybeSingle();
  if (poError || !poRow) {
    if (poError) console.error("Error fetching PO detail:", poError);
    return null;
  }

  let abstractRow: { pr_id?: string; winning_b_id?: string } | null = null;
  if (poRow.a_id) {
    const { data, error } = await client
      .schema("bac")
      .from("abstract")
      .select("pr_id, winning_b_id")
      .eq("a_id", poRow.a_id)
      .maybeSingle();
    if (error) {
      console.error("Error fetching abstract for PO:", error);
    }
    abstractRow = data;
  }

  let rcId: string | null = null;
  if (abstractRow?.pr_id) {
    const { data, error } = await client
      .schema("gse")
      .from("purchase_request")
      .select("rc_id")
      .eq("pr_id", abstractRow.pr_id)
      .maybeSingle();
    if (error) {
      console.error("Error resolving RC from PR:", error);
    }
    rcId = data?.rc_id ?? null;
  }

  return {
    po_id: poRow.po_id,
    po_no: poRow.po_no,
    po_total_amount: poRow.po_total_amount ?? 0,
    a_id: poRow.a_id ?? null,
    winning_b_id: abstractRow?.winning_b_id ?? null,
    rc_id: rcId,
  };
};
...
export const upsertBudgetPendingDetailFromPO = async (
  poId: string,
): Promise<{ success: boolean; error?: string }> => {
  if (!isSupabaseConfigured() || !supabase)
    return { success: false, error: "Supabase is not configured" };
  ...
  // Resolve PR rc_id through PO -> Abstract -> Purchase Request.
  const { data: poRow, error: poError } = await client
    .schema("bac")
    .from("purchase_order")
    .select("a_id")
  ...
  if (poError || !poRow?.a_id) {
    return {
      success: false,
      error: poError?.message || "Unable to resolve abstract from PO.",
    };
  }

  const { data: abstractRow, error: abstractError } = await client
    .schema("bac")
    .from("abstract")
    .select("pr_id")
    .eq("a_id", poRow.a_id)
  ...
  if (abstractError || !abstractRow?.pr_id) {
    return {
      success: false,
      error: abstractError?.message || "Unable to resolve PR from abstract.",
    };
  }
  ...
  if (prError || !prRow?.rc_id) {
    return {
      success: false,
      error: prError?.message || "Unable to resolve responsibility center.",
    };
  }

  // Find matching RC-GLA connection(s) for this responsibility center.
  ...
  if (rcGlaError || !rcGlaRows || rcGlaRows.length === 0) {
    return {
      ...
      error:
        rcGlaError?.message ||
        "No RC-GLA mapping found for the PO responsibility center.",
    };
  }

  const rcGlaIds = rcGlaRows.map((row) => row.id as string);

  // Prefer account with highest remaining budget if multiple mappings exist.
  ...
  // Upsert-like behavior by po_id in pending_details.
...
/** Get total awarded (consumed) amount for a PR from all awarded abstracts + child PRs */
export const getPRConsumedAmount = async (prId: string): Promise<number> => {
  if (!isSupabaseConfigured() || !supabase) return 0;

  let totalConsumed = 0;

  // 1. Get all abstracts for this PR that have a winner
  ...
  // 2. Get all child PRs that use this PR's budget (parent_pr_id = prId)
  const { data: childPRs, error: childError } = await (
    supabase as NonNullable<typeof supabase>
  )
    .schema("gse")
    .from("purchase_request")
    .select("pr_total_amount")
    .eq("parent_pr_id", prId);

  if (!childError && childPRs && childPRs.length > 0) {
    for (const child of childPRs) {
      totalConsumed += child.pr_total_amount || 0;
    }
  }
 ...
 *  Excludes child PRs (those with parent_pr_id) - only root PRs can be budget sources. */
...
> => {
  ...
  // Get all approved PRs that are NOT child PRs (parent_pr_id is null)
