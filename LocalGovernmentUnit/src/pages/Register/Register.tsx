...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Create the auth user via the backend (admin-confirmed signup,
      //    profile synced into public.users, role defaulted to "user").
      ...
      // 2. Add a pending_users row so the admin-approval workflow still
      //    works. The backend created the auth user but we keep the
      //    LGU-specific approval gate in front of first login.
      const pending = await createPendingUser(username, normalizedEmail);
      if (!pending.success) {
        // Non-fatal — the account exists; user just won't be gated by
        // pending_users. Surface as a warning rather than blocking.
        console.warn(
          "Pending-users record failed to create:",
          pending.error,
        );
      }
