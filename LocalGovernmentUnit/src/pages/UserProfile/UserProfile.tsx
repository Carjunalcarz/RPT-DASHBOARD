...
const UserProfile = () => {
  ...
  // Assigned RBAC roles (from user_roles via /rbac/me) — the real roles the
  // user holds, as opposed to the legacy single `users.role` string. Fall back
  // to that legacy field only when no RBAC role is assigned.
  const { roles, isSuperAdmin } = useRBAC();
  const roleDisplay =
    roles.length > 0
      ? roles.map((r) => r.roleName || r.roleCode).filter(Boolean).join(", ")
      : isSuperAdmin
        ? "Super Admin"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
        ...
        <div className="bg-surface border border-border rounded-2xl p-6 text-center h-fit">
          ...
          {/* Role Badge(s) — assigned RBAC roles */}
          <div className="flex flex-wrap items-center justify-center gap-1.5 mt-3">
            {roles.length > 0 ? (
              roles.map((r) => (
                <span
                  key={r.roleCode ?? r.roleName ?? "role"}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-success/10 text-success rounded-full text-xs font-semibold"
                >
              ...
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-success/10 text-success rounded-full text-xs font-semibold">
                <Shield className="w-3.5 h-3.5" />
                {roleDisplay}
              </span>
            )}
