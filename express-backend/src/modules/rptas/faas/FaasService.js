...
class FaasService {
  constructor({ supabasePrisma, logger, rptMastService, rptAssService }) {
    ...
    this.logger = logger;
    this.rptMastService = rptMastService;
    this.rptAssService = rptAssService;
  }
  /**
  ...
  async validateTransaction(data, currentId = null, existingRecord = null) {
    ...
    if (tdn) {
      checks.push(
        this.supabasePrisma.faasRecord.findFirst({
          where: {
            tdn: tdn,
      ...
      if ((tdn || pin) && !skipMssqlCheck) {
          checks.push(
              this.rptMastService.checkDuplicate(tdn, pin).then(mssqlCheck => {
                     ...
                     if (pOldTdn && mssqlCheck.tdnRecord.TDN === pOldTdn && tdn === pOldTdn) {
                         ...
                         // Rule 3: TDN Update Validation
                          ...
                          if (mssqlCheck.tdnRecord.PIN !== pin) {
                             ...
                             this.logger.warn(`Legacy Conflict Warning: TDN ${tdn} exists in MSSQL but allowing save to Supabase.`);
                          } else {
  ...
  async saveDraft(data, userEmail, id = null, userId = null) {
    try {
      ...
      if (id) {
        existingRecord = await this.supabasePrisma.faasRecord.findUnique({ where: { id } });
      } else if (data.id && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(data.id)) {
        existingRecord = await this.supabasePrisma.faasRecord.findUnique({ where: { id: data.id } });
      }
      
      if (!existingRecord && tdn) {
        existingRecord = await this.supabasePrisma.faasRecord.findUnique({ where: { tdn } });
      }
      ...
      if (existingRecord) {
        ...
        action = 'UPDATE_DRAFT';
        record = await this.supabasePrisma.faasRecord.update({
        ...
        record = await this.supabasePrisma.faasRecord.create({
          data: {
      ...
      this.logAudit(action, record.id, { tdn }, userEmail, userId).catch(err => {
          this.logger.warn(`Background audit log failed for ${action}:`, err.message);
      });

      // Log migration/sync event if this originated from MSSQL (TDN exists) (Non-blocking)
      if (tdn) {
          this.supabasePrisma.migrationLog.create({
              data: {
                  sourceId: tdn,
                  targetId: record.id,
                  status: 'synced',
              ...
              this.logger.warn(`Background migration log failed for ${tdn}:`, err.message);
          });
      }

      return record;
    } catch (error) {
      this.logger.error('Error in FaasService.saveDraft:', error);
  ...
  async submitForReview(id) {
    try {
      const record = await this.supabasePrisma.faasRecord.update({
        where: { id },
        data: {
      ...
      this.logger.error('Error in FaasService.submitForReview:', error);
      const statusCode = error.statusCode || 500;
      throw new AppError(error.message, statusCode);
    }
  }
  ...
  async getRecord(id) {
    try {
      const record = await this.supabasePrisma.faasRecord.findUnique({
        where: { id }
      });
      return record;
    } catch (error) {
      this.logger.error('Error in FaasService.getRecord:', error);
  ...
  async cancelTransaction(id, userEmail, userId) {
    try {
      ...
      // Delete the record
      await this.supabasePrisma.faasRecord.delete({
        where: { id }
      });
      ...
      this.logger.error('Error in FaasService.cancelTransaction:', error);
  ...
  async deleteRecord(id, userEmail = null, userId = null) {
    try {
      const record = await this.supabasePrisma.faasRecord.delete({
        where: { id }
      });
      ...
      this.logger.error('Error in FaasService.deleteRecord:', error);
      throw new AppError(error.message, 500);
    }
  }
  ...
  async updateStatus(id, status, remarks, userEmail, userId) {
    try {
        ...
        if (status === 'pending-municipal') {
            this.logger.info(`[NOTIFICATION] Alerting Municipal Assessors: New record ${currentRecord.tdn || id} pending review.`);
        } else if (status === 'pending-provincial') {
            this.logger.info(`[NOTIFICATION] Alerting Provincial Assessors: Record ${currentRecord.tdn || id} approved by Municipal, pending Provincial review.`);
        } else if (status === 'approved') {
            this.logger.info(`[NOTIFICATION] Alerting Owner/Municipal: Record ${currentRecord.tdn || id} fully APPROVED.`);
        } else if (status.includes('rejected')) {
            this.logger.info(`[NOTIFICATION] Alerting Submitter: Record ${currentRecord.tdn || id} REJECTED.`);
        }

        return record;
    } catch (error) {
        this.logger.error('Error in FaasService.updateStatus:', error);
        throw new AppError(error.message, error.statusCode || 500);
    }
  }
  ...
  async batchUpdateStatus(ids, status, remarks, userEmail, userId) {
    try {
        ...
        if (status === 'pending-provincial' || status === 'approved' || (typeof status === 'string' && status.includes('rejected'))) {
            const rows = await this.supabasePrisma.faasRecord.findMany({
            ...
            await this.supabasePrisma.$transaction(async (tx) => {
                if (status === 'pending-provincial') {
        ...
        this.logger.error('Error in FaasService.batchUpdateStatus:', error);
        throw new AppError(error.message, error.statusCode || 500);
    }
  }
  ...
  async logAudit(action, recordId, details, userEmail, userId) {
    try {
        await this.supabasePrisma.auditLog.create({
            data: {
                tableName: 'faas_records',
                recordId: recordId,
                action: action,
  ...
  async getTdnHistory(id) {
    try {
      ...
      let faas = null;
      if (!masterPropertyId) {
        faas = await this.supabasePrisma.faasRecord.findUnique({ where: { id } });
        if (!faas) return [];
      ...
      const rows = await this.supabasePrisma.$queryRawUnsafe(
        `SELECT
           h.id,
           h.tdn,
      ...
      this.logger.error('Error in FaasService.getTdnHistory:', error);
      throw new AppError(error.message, 500);
    }
  }
  ...
  async bulkMigrate(properties, migrationType, userEmail, userId, skipExisting = false) {
    this.logger.info(`Starting bulk migration for ${properties.length} properties. Type: ${migrationType}, SkipExisting: ${skipExisting}`);
    ...
    try {
      ...
      if (skipExisting) {
        ...
        const existing = await this.supabasePrisma.faasRecord.findMany({
          where: { tdn: { in: tdns } },
          select: { tdn: true }
        });
        ...
        for (const property of properties) {
          try {
            ...
            if (skipExisting && tdnsToSkip.includes(tdn)) {
              this.logger.info(`Skipping existing property ${tdn} during bulk migration`);
              results.push({
                id: property.id,
                tdn: tdn,
                status: 'skipped',
            ...
            if (!propertyData.assessments || !Array.isArray(propertyData.assessments) || propertyData.assessments.length === 0) {
              this.logger.info(`Fetching assessments for property ${tdn} from MSSQL`);
              const assResult = await this.rptAssService.getAll({ filters: { TDN: tdn } });
            ...
            this.logger.error(`Failed to migrate property ${property.tdn || 'unknown'}: ${err.message}`);
      ...
      this.logger.error(`Bulk migration transaction failed: ${error.message}`);
  ...
  async checkExistingTdns(tdns) {
    try {
      ...
      this.logger.error('Error checking existing TDNs:', error);
  ...
  async listRecords({ status, page = 1, limit = 100, searchField, filterValue }) {
    try {
      ...
      const [records, total] = await Promise.all([
        this.supabasePrisma.faasRecord.findMany({
          where,
          skip,
          take,
          orderBy: { updatedAt: 'desc' }
        }),
        this.supabasePrisma.faasRecord.count({ where })
      ]);
      ...
      this.logger.error('Error in FaasService.listRecords:', error);
      throw new AppError(error.message, 500);
    }
  }
}
