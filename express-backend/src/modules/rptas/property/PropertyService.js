...
class PropertyService {
  constructor({ mssqlPrisma, supabasePrisma, logger }) {
    ...
    try {
      ...
      if (user && user.role === 'admin') {
        ...
        if (!targetMunicipality && user.municipalityCode !== 'ADN' && user.municipalityCode !== 'ALL') {
          this.logger.warn(`User ${user.email} has no municipalityCode assigned. Returning empty result.`);
          return {
            data: [],
            pagination: {
              page: Number(page),
...
`;
      ...
      this.logger.info('Executing RPTAS_AGUSAN migration query via MSSQL driver...');
      ...
      const records = result.recordset.map(record => {
          ...
          if (record.trees_json) {
              try {
                  ...
                  this.logger.warn(`Failed to parse trees_json for TDN ${record.TDN}`, e);
              }
          }
      ...
      this.logger.info(`Query executed successfully. Records found: ${records.length}`);
      ...
      this.logger.error('Error executing RPTAS_AGUSAN migration query:', error);
  ...
  async getMastExtn(tdn) {
    try {
      ...
      this.logger.error('Error fetching MASTEXTN data:', error);
  ...
  async updateSignatory(tdn, data) {
    try {
      ...
      this.logger.error('Error updating Signatories:', error);
