...
const { validatePayorPayload, normalizeIdKey } = require('../../../services/payorValidation');
const { verifyPayorIdentity } = require('../../../services/identityVerificationService');
...
class PayorService {
  ...
  toRow = (payload) => ({
    firstName: this.normalize(payload.firstName),
    lastName: this.normalize(payload.lastName),
    address: this.normalize(payload.address),
    idType: this.normalize(payload.idType).toLowerCase(),
    idNumber: this.normalize(payload.idNumber).toUpperCase(),
    contact: payload.contact || {},
  });

  searchPayors = async ({ q, limit = 10 }) => {
  const query = this.normalize(q);
  const take = Math.min(Math.max(Number(limit) || 10, 1), 25);
  if (!query) return [];
  if (!(await this.hasPayorsTable())) {
  ...
  createPayor = async ({ user, payload }) => {
  ...
  bulkCreatePayors = async ({ user, rows }) => {
  ...
  rows.forEach((r, idx) => {
    ...
    seen.add(key);
    normalizedRows.push(this.toRow(r));
  });
