...
router.get('/stats', protect, async (req, res) => {
  try {
    const [propertiesRow] = await supabasePrisma.$queryRawUnsafe(
      `SELECT COUNT(*)::int as count FROM public.rpt_property`
    );
    const totalProperties = Number(propertiesRow?.count || 0);

    const delinquentRow = await supabasePrisma.$queryRawUnsafe(
      `SELECT COUNT(*)::int as count FROM public.rpt_property WHERE payment_status = 'unpaid'::public.rpt_payment_status`
    );
    ...
    try {
      ...
      const pendingRow = await supabasePrisma.$queryRawUnsafe(
        `SELECT COUNT(*)::int as count FROM public.orders_of_payment WHERE status = 'pending'`
      );
      ...
      const rows = await supabasePrisma.$queryRawUnsafe(
        `
          ...
          FROM (
            ...
            FROM public.treasury_payment_exports
