      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-1 bg-slate-100 dark:bg-slate-800 p-2 rounded border border-slate-200 dark:border-slate-700">
            ...
            <button onClick={handleSave} disabled={!isLocalFormEnabled} className="px-3 py-1.5 text-xs bg-blue-600 text-white hover:bg-blue-700 rounded shadow-sm flex items-center gap-1 disabled:opacity-50">
              <Save size={14} /> Save
            </button>
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {records.length === 0 ? (
                    ...
                    records.map((r, i) => (
                        <tr key={i} onClick={() => handleRowSelect(r)} className={`cursor-pointer hover:bg-blue-50 dark:hover:bg-slate-800 ${selectedRecord === r ? 'bg-blue-100 dark:bg-blue-900/30' : ''}`}>
