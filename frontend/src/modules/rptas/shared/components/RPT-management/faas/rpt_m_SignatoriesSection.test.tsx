...
describe('SignatoriesSection Auto-Prefill', () => {
  ...
  it('shows the Auto-Prefill button when in edit mode', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('2024 - Test Template 2024')).toBeInTheDocument();
    });
  });

  it('shows the Auto-Prefill button (disabled) when a transaction is active but not editing', async () => {
    ...
    const prefillBtn = await screen.findByTestId('btn-auto-prefill');
    expect(prefillBtn).toBeInTheDocument();
    expect(prefillBtn).toBeDisabled();
  });
