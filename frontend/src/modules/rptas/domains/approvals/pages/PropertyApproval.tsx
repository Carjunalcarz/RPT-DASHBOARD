...
import PropertyDetailsView from '@/modules/rptas-standalone/shared/components/data-entry/faas/PropertyDetailsView';
...
import { PropertyRecord } from '@/modules/rptas-standalone/shared/components/data-entry/faas/RealPropertyDataEntry';
    ...
    try {
        ...
        try {
            ...
            if (faasRecord) {
                ...
                const currentTdn = (faasRecord as any).tdn || innerData.tdn || innerData.TDN || '';
                ...
                mappedRecord = {
                    ...
                    ARP: currentTdn,
                    PIN: currentPin,
                    status: faasRecord.status as any,
                    owner: innerData.owner || innerData.owner_name || 'N/A',
                    barangay: innerData.barangay || 'N/A',
                    OWNER_NO: innerData.OWNER_NO || '',
                    // Signatory Fields
                    recApproval: (innerData as any).Rec_Approval,
                    recApprovalPos: (innerData as any).Rec_ApprovalPos,
                    recAppDate: (innerData as any).Rec_AppDate,
                    approved: (innerData as any).Approved,
                    approvedPos: (innerData as any).ApprovedPos,
                    approvedDate: (innerData as any).ApprovedDate,
        ...
        if (!mappedRecord) {
            ...
            if (mastResponse.data && mastResponse.data.length > 0) {
                ...
                mappedRecord = {
                    ...
                    ARP: currentTdn,
                    PIN: currentPin,
                    status: (item as any).status as any,
                    owner: innerData.owner || innerData.owner_name || 'N/A',
                    barangay: innerData.barangay || 'N/A',
                    OWNER_NO: innerData.OWNER_NO || '',
                    recApproval: (innerData as any).Rec_Approval,
        ...
        if (mappedRecord) {
            ...
            if ((mappedRecord as any).assessments && (mappedRecord as any).assessments.length > 0) {
                ...
                const assData = await getRptAssByTdn(mappedRecord.TDN || mappedRecord.tdn);
    ...
    if (isConfirmed) {
        ...
        try {
            ...
            if (isFaasRecord) {
                ...
                await updateSignatory(record.TDN || record.tdn, { 
                    status: nextStatus,
                    Approved: user?.name,
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
        ...
        if (isFaasRecord) {
             ...
             await updateSignatory(record.TDN || record.tdn, updateData);
        }
    ...
    if (isConfirmed) {
      ...
      try {
        ...
        if (isFaasRecord) {
             ...
             await updateSignatory(record.TDN || record.tdn, { status: 'pending-municipal' });
        }
    ...
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-900">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-auto bg-slate-50 dark:bg-slate-900 relative">
            ...
            <div className="absolute inset-0 bg-transparent z-50 cursor-not-allowed" title="This record is currently locked for review."></div>
        </div>
                    ...
                    <div className="relative pl-4 border-l-2 border-slate-200 dark:border-slate-600 space-y-4">
                        ...
                        {(record as any)?.approved && (
                            <div className="relative">
                                ...
                                <div className="text-xs text-slate-600 dark:text-slate-300">{(record as any).approved}</div>
                                <div className="text-xs text-slate-400">{(record as any).approvedDate ? new Date((record as any).approvedDate).toLocaleDateString() : '-'}</div>
                            </div>
                        )}
                        {(record as any)?.recApproval && (
                            <div className="relative">
                                ...
                                <div className="text-xs text-slate-600 dark:text-slate-300">{(record as any).recApproval}</div>
                                <div className="text-xs text-slate-400">{(record as any).recAppDate ? new Date((record as any).recAppDate).toLocaleDateString() : '-'}</div>
                            </div>
                        )}
                    </div>
                </div>
