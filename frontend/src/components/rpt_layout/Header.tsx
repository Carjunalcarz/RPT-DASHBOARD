...
import { Menu, Search, Bell, Sun, Moon, User, Settings, LogOut, Palette, ArrowLeftRight } from 'lucide-react';
...
const Header: React.FC = () => {
  ...
  const { layoutMode, toggleLayout } = useLayoutToggle();
  ...
  return (
    <header
      data-testid="header"
      className="fixed top-0 right-0 left-0 h-16 bg-white dark:bg-slate-900 shadow-sm z-40"
    >
      <div className="flex items-center justify-between h-full px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title="Toggle Layout"
          >
            <ArrowLeftRight size={20} className="text-slate-600 dark:text-slate-400" />
            <span className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap pointer-events-none transition-opacity z-50">
              Switch to {layoutMode === 'main' ? 'RPT Layout' : 'Main Layout'}
            </span>
          </button>
