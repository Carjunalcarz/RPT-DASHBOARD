...
const AdjustmentTab: React.FC<AdjustmentTabProps> = ({ buildingId, isFormEnabled, initialAdjustments = [], structures = [] }) => {
  const [records, setRecords] = useState<BldgAdjRecord[]>(initialAdjustments);

  useEffect(() => {
    // Force update when initialAdjustments changes, ensuring UI reflects API data
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm('Delete this adjustment record?')) {
      try {
        ...
        // Fallback to local delete if API fails (for demo/dev without full backend)?
        // Better to alert user.
        alert('Failed to delete adjustment record.');
      }
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const recordData: Partial<BldgAdjRecord> = {
        ...
        // Ensure SeqNo exists for updates, or generated for inserts if needed
      ...
      alert('Failed to save adjustment record.');
    }
  };
  ...
  const totalAdjustment = records.reduce((sum, r) => sum + (r.Acc_Dep || 0), 0);
            ...
            {records.length === 0 ? (
              ...
              records.map((record, index) => (
                <tr
                  ...
                  data-testid={`adjustment-row-${record.SeqNo}`}
                >
