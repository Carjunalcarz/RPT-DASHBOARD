...
const StructureTab: React.FC<StructureTabProps> = ({ buildingId, isFormEnabled, initialStructures = [] }) => {
  const { headerColor, headerColorDark } = useThemeColor();
  const [records, setRecords] = useState<BldgStrucRecord[]>(initialStructures);
  
  // Use a ref to track the previous initialStructures to prevent infinite loops
  // if the parent passes a new array reference with the same content
  const prevInitialStructuresRef = useRef<BldgStrucRecord[]>(initialStructures);

  useEffect(() => {
    // Only update if the content has actually changed
    // This prevents "Maximum update depth exceeded" if parent re-renders and passes new array reference
  ...
  const handleEdit = () => {
    ...
    setIsEditing(true);
    setIsAdding(false);
    // Ensure form data is populated when entering edit mode
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm('Delete this structure record?')) {
      try {
        ...
        if (success) {
          setRecords(prev => prev.filter(r => r.FloorOrd !== selectedRecord.FloorOrd));
          setSelectedRecord(null);
          handleAdd(); // Reset form
        }
      } catch (error) {
        console.error('Error deleting structure:', error);
        alert('Failed to delete structure record.');
      }
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (isAdding) {
        const newRecord = await createBldgStruc(recordData);
        if (newRecord) {
        ...
        if (!selectedRecord?.FloorOrd) return;
        const updatedRecord = await updateBldgStruc(buildingId, selectedRecord.FloorOrd, recordData);
        if (updatedRecord) {
          setRecords(prev => prev.map(r => r.FloorOrd === selectedRecord.FloorOrd ? updatedRecord : r));
      ...
      alert('Failed to save structure record.');
    }
  };
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
            {records.length === 0 ? (
              ...
              records.map((record, index) => (
                ...
                >
                  <td className="px-3 py-1.5">{record.Struc_type || '-'}</td>
        ...
        <div className="p-3 grid grid-cols-3 gap-x-6 gap-y-2">
          ...
          <div className="space-y-2">
            <div className="flex items-center">
              ...
              >
                {structureTypeOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
              </select>
            </div>
            <div className="flex items-center">
